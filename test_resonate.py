...
class TestResonateLauncher(unittest.TestCase):
    ...
    def test_launcher_help(self):
        ...
        result = subprocess.run(
            ["python", str(self.launcher_path), "--help"],
            capture_output=True,
            text=True
        )
    ...
    def test_resonate_status(self):
        ...
        result = subprocess.run(
            ["python", str(self.launcher_path), "status"],
            capture_output=True,
            text=True
        )
        self.assertEqual(result.returncode, 0)
    ...
    def test_cli_resonate_status(self):
        ...
        result = subprocess.run(
            ["python", str(self.cli_path), "resonate", "status"],
            capture_output=True,
            text=True
        )
        self.assertEqual(result.returncode, 0)
    ...
    def test_resonate_help(self):
        ...
        result = subprocess.run(
            ["python", str(self.cli_path), "resonate", "--help"],
            capture_output=True,
            text=True
        )
        self.assertEqual(result.returncode, 0)
...
def run_integration_test():
    ...
    try:
        result = subprocess.run(
            ["python", "ballsdeepnit_cli.py", "resonate", "status"],
            capture_output=True,
            text=True,
            timeout=10
        )
