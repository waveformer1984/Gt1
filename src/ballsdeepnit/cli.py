...
def beacon(host: str, port: int) -> None:
    """Launch visual hardware verification beacon."""
    logger = get_logger(__name__)
    try:
        from .dashboard.hardware_beacon import hardware_beacon
        ...
        import asyncio
        asyncio.run(hardware_beacon.start_beacon(host=host, port=port))
        
    except KeyboardInterrupt:
        logger.info("Hardware beacon stopped by user")
    except Exception as e:
        logger.error(f"Hardware beacon error: {e}", exc_info=True)
        sys.exit(1)

@cli.command()
@click.option('--host', default=settings.HOST, help='Host to bind to')
@click.option('--port', default=settings.DASHBOARD_PORT, help='Port to bind to')
@click.option('--integrated', is_flag=True, help='Run integrated SIDD dashboard')
def dashboard(host: str, port: int, integrated: bool) -> None:
    """Run the performance-optimized dashboard."""
    logger = get_logger(__name__)
    
    try:
        if integrated:
            # Start integrated SIDD dashboard
            from .dashboard.sidd_dashboard import dashboard as sidd_dashboard
            import uvicorn
            ...
            async def websocket_endpoint(websocket: WebSocket):
                ...
                try:
                    while True:
                        data = await sidd_dashboard.get_dashboard_data()
                        await websocket.send_text(json.dumps(data))
                        await asyncio.sleep(update_interval)  # Update based on user-defined interval
                except Exception as e:
            ...
            async def get_dashboard():
                return await sidd_dashboard.get_dashboard_data()
            
            @app.get("/api/hardware")
            async def get_hardware_specs():
                from .dashboard.hardware_monitor import frank_hardware
                return await frank_hardware.get_complete_hardware_specs()
            
            @app.get("/health")
            async def health_check():
                return {"status": "healthy", "service": "SIDD Dashboard"}
            
            logger.info(f"🎛️ Starting SIDD Integrated Dashboard on {host}:{port}")
            logger.info(f"🌐 Dashboard: http://{host}:{port}")
            logger.info(f"📡 WebSocket: ws://{host}:{port}/ws/dashboard")
            logger.info(f"🔗 API: http://{host}:{port}/api/dashboard")
