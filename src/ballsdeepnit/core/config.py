...
class MCPSettings(BaseSettings):
    ...
    MAX_CONNECTIONS_PER_SERVER: int = Field(default=10)
    ...
    ENABLE_WEB_SEARCH_SERVER: bool = Field(default=True)
    ENABLE_GITHUB_SERVER: bool = Field(default=True)
    
    # API Keys for MCP servers
    BRAVE_API_KEY: str = Field(
        default="",
        description="Brave Search API key. Required if ENABLE_WEB_SEARCH_SERVER is True."
    )
    GITHUB_TOKEN: str = Field(
        default="",
        description="GitHub API token. Required if ENABLE_GITHUB_SERVER is True."
    )
    OPENAI_API_KEY: str = Field(
        default="",
        description="OpenAI API key. Required if using OpenAI services."
    )
    
    # Performance optimization
    ENABLE_FUNCTION_OPTIMIZATION: bool = Field(default=True)
